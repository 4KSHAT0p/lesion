...
class LesionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            mask = TF.to_tensor(mask)  # Convert mask to tensor
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        # Encoder
        enc1 = self.enc1(x)
        enc2 = self.enc2(self.encoder.maxpool(enc1))
        enc3 = self.enc3(enc2)
        enc4 = self.enc4(enc3)
        enc5 = self.enc5(enc4)
...
resultsTEST = evaluate_on_test_data(
    "lesion_segmentation_model_EX.pth", test_image_path, test_mask_path, device, transform
)

# Fix the bug in the original code: val_image_path was used twice
print("RESULTS ON VALIDATION DATA")
resultsVAL = evaluate_on_test_data(
    "lesion_segmentation_model_EX.pth